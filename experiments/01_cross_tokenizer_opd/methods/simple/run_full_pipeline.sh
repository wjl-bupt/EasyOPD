...
BENCH_DIR="/apdcephfs_cq8/share_1324356/shinejiesun/workspace/EasyOPD/experiments"
EXP_DIR="${BENCH_DIR}/01_cross_tokenizer_opd"
...
TEACHER_SFT_DIR="${EXP_DIR}/checkpoints/teacher_sft_phi4mini"
TEACHER_SFT_MODEL="${TEACHER_SFT_DIR}/final"
SFT_DATA_DIR="${EXP_DIR}/data/teacher_sft_data"
...
if [ ! -f "${SFT_DATA_DIR}/teacher_sft_train.jsonl" ]; then
    ${PYTHON} ${BENCH_DIR}/_shared/scripts/gen_teacher.py --teacher_model ${TEACHER_MODEL} --tp 4
...
if [ ! -d "${TEACHER_SFT_MODEL}" ]; then
    /opt/conda/envs/OpenAgentRL-sj/bin/accelerate launch \
        ...
        ${BENCH_DIR}/_shared/scripts/run_teacher_sft.py \
...
${PYTHON} ${BENCH_DIR}/_shared/scripts/evaluate_model.py --model_path ${LOCAL_TEACHER_SFT} --model_name "Teacher_SFT_Phi4mini" --tensor_parallel_size 1 --dp_size 8 --benchmarks math500
${PYTHON} ${BENCH_DIR}/_shared/scripts/evaluate_model.py --model_path ${LOCAL_TEACHER_SFT} --model_name "Teacher_SFT_Phi4mini" --tensor_parallel_size 1 --dp_size 8 --benchmarks gsm8k
...
${PYTHON} -m verl.trainer.main_ppo \
    ...
    custom_reward_function.path="${BENCH_DIR}/_shared/scripts/reward_fn.py" \
...
${PYTHON} ${BENCH_DIR}/_shared/scripts/merge_fsdp.py --ckpt_dir ${SIMPLE_ACTOR_CKPT} --base_model ${TEACHER_SFT_MODEL} --output_dir ${SIMPLE_CKPT}/merged_hf
...
echo "[$(date)] Copy done."
${PYTHON} ${BENCH_DIR}/_shared/scripts/evaluate_model.py --model_path ${LOCAL_SIMPLE} --model_name "Simple_on_TeacherSFT" --tensor_parallel_size 1 --dp_size 8 --benchmarks math500
${PYTHON} ${BENCH_DIR}/_shared/scripts/evaluate_model.py --model_path ${LOCAL_SIMPLE} --model_name "Simple_on_TeacherSFT" --tensor_parallel_size 1 --dp_size 8 --benchmarks gsm8k

echo "[$(date)] ===== STEP 7: Run SimCT on Teacher SFT model ====="
${RAY} stop --force 2>/dev/null || true
sleep 3
${RAY} start --head --disable-usage-stats --num-cpus=64 --num-gpus=8 --include-dashboard=false
SIMCT_CKPT="${EXP_DIR}/simct/checkpoints/simct_on_teacher_sft"
mkdir -p ${SIMCT_CKPT}
${PYTHON} -m verl.trainer.main_ppo \
    algorithm.adv_estimator=grpo algorithm.use_kl_in_reward=True \
    "data.train_files=['${SFT_DATA_DIR}/train_for_rl.parquet']" \
...
${PYTHON} ${BENCH_DIR}/_shared/scripts/merge_fsdp.py --ckpt_dir ${SIMCT_ACTOR_CKPT} --base_model ${TEACHER_SFT_MODEL} --output_dir ${SIMCT_CKPT}/merged_hf
...
echo "[$(date)] Copy done."
${PYTHON} ${BENCH_DIR}/_shared/scripts/evaluate_model.py --model_path ${LOCAL_SIMCT} --model_name "SimCT_on_TeacherSFT" --tensor_parallel_size 1 --dp_size 8 --benchmarks math500
${PYTHON} ${BENCH_DIR}/_shared/scripts/evaluate_model.py --model_path ${LOCAL_SIMCT} --model_name "SimCT_on_TeacherSFT" --tensor_parallel_size 1 --dp_size 8 --benchmarks gsm8k
