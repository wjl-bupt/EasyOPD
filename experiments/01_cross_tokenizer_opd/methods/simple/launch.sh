...
# ============================================================
# Simple (Cross-Tokenizer OPD) — KDFlow-aligned config
#
...
#   2. (Re)start Ray
#   3. GRPO + cross-tokenizer KD (simple loss) via verl.trainer.main_ppo
...
#     EXPECTED_FINAL_STEP = 9900 / 64 ≈ 155
#     ROLLOUT_GPU_MEM_UTIL  = 0.55  (vLLM gpu_memory_utilization)
#     TEACHER_GPU_MEM_UTIL  = 0.55  (SGLang mem_fraction_static)
#     SIMPLE_TEACHER_NUM_GPUS_PER_ACTOR = 0.2 (KDFlow convention; binding via base_gpu_id)
...
#   total_epochs                = 1
#   save_freq                   = 75            (-> ckpt at step 75 and epoch-end)
...
METHOD_DIR="${EXP_DIR}/methods/simple"
...
# Distillation knobs (mirror KDFlow `simple` loss = cross-tokenizer reverse KL on overlap sub-vocab)
DISTILL_LOSS_MODE=simple
...
if [ "${TEACHER_LAYOUT}" = "shared" ]; then
    ...
    #   - sidecar wakes SGLang only inside _maybe_build_simple_xtok_batch.
...
# ============================================================
# Step 3: GRPO + Simple cross-tokenizer KD training
...
echo "[$(date)] ===== Step 3: GRPO + Simple Cross-Tok KD Training ====="
...
${PYTHON} -m verl.trainer.main_ppo \
    ...
    +actor_rollout_ref.actor.policy_loss.simple_kl_direction=${KL_DIRECTION} \
    ...
    trainer.project_name=easyopd-simple \
...
# ============================================================
echo "[$(date)] ===== Step 5: Evaluating Simple checkpoint(s) ====="
