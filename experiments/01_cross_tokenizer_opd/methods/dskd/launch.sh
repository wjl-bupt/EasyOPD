...
# ============================================================
# DSKD (Dual-Space Knowledge Distillation) Cross-Tokenizer OPD launch script.
#
# Sibling of methods/simct/launch.sh. DSKD initializes two projectors
# (t2s: teacher_hidden -> student space; s2t: student_hidden -> teacher
# space) via closed-form pinv and minimises a combination of t2s_kd /
# t2s_ce / s2t_kd losses. Three token-alignment branches are supported:
#   - identical : teacher/student vocab identical, direct per-token loss.
#   - eta       : greedy character-level alignment over response tokens.
#   - cma       : cross-modal-attention soft-alignment (single-sample).
#
# Architectural simplification vs KDFlow: in this EasyOPD port the
# projectors are FROZEN at their pinv-initialised values (we do not
# inject extra param groups into verl's actor optimizer). This
# reproduces the bulk of the KD signal at step 0; the student lm_head
# itself absorbs the remaining adaptation through the t2s_kd term.
# To re-enable trainable projectors a future revision would need to
# patch verl's optimizer setup.
#
...
# What differs:
#   * METHOD=dskd, RUN_NAME=dskd_phi4mini, DISTILL_LOSS_MODE=dskd.
#   * actor_rollout_ref.actor.policy_loss.loss_mode=dskd.
#   * +distillation.distillation_loss.{dskd_token_align, dskd_topk_vocab,
#                                     dskd_projector_grad_clamp,
#                                     dskd_projector_lr_scale}.
#   * `simple_loss_clamp` is RETAINED.
# ============================================================
...
METHOD_DIR="${EXP_DIR}/methods/dskd"
...
if [ "${SPEED_TIER}" = "safe" ]; then
    ...
    PPO_MAX_TOKEN_LEN_PER_GPU=8193  # DSKD projectors need extra mem vs ULD/ALM
...
DISTILL_LOSS_MODE=dskd
KL_DIRECTION=reverse              # unused; kept for hydra compat
USE_POLICY_GRADIENT=False         # kd_ratio=1.0 -> pure distillation

# DSKD-specific KD hyperparameters (KDFlow defaults).
# token_align: identical | eta | cma (eta is the typical cross-tokenizer setting).
DSKD_TOKEN_ALIGN=${DSKD_TOKEN_ALIGN:-eta}
DSKD_TOPK_VOCAB=${DSKD_TOPK_VOCAB:--1}                  # -1 sentinel = use full overlap
DSKD_PROJECTOR_GRAD_CLAMP=${DSKD_PROJECTOR_GRAD_CLAMP:-1.0}   # only used if projector becomes trainable
DSKD_PROJECTOR_LR_SCALE=${DSKD_PROJECTOR_LR_SCALE:-1.0}
...
echo "[$(date)] ===== Step 3: GRPO + DSKD Cross-Tok KD Training ====="
...
${PYTHON} -m verl.trainer.main_ppo \
    ...
    +distillation.distillation_loss.dskd_token_align=${DSKD_TOKEN_ALIGN} \
    +distillation.distillation_loss.dskd_topk_vocab=${DSKD_TOPK_VOCAB} \
    +distillation.distillation_loss.dskd_projector_grad_clamp=${DSKD_PROJECTOR_GRAD_CLAMP} \
    +distillation.distillation_loss.dskd_projector_lr_scale=${DSKD_PROJECTOR_LR_SCALE} \
    trainer.balance_batch=True \
    'trainer.logger=["console"]' \
    trainer.project_name=easyopd-dskd \
...
# ============================================================
echo "[$(date)] ===== Step 5: Evaluating DSKD checkpoint(s) on all benchmarks ====="
