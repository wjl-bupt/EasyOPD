...
# ============================================================
# ALM (Approximate Likelihood Matching) Cross-Tokenizer OPD launch script.
#
# Sibling of methods/simct/launch.sh — only the loss mode and method-
# specific hydra overrides differ. ALM aligns teacher/student response
# tokens into chunks via cumulative tokenizer.decode comparison and
# minimises a binarised f-divergence (KL by default; TVD optional)
# between chunk-level log-probabilities.
#
...
# What differs:
#   * METHOD=alm, RUN_NAME=alm_phi4mini, DISTILL_LOSS_MODE=alm.
#   * actor_rollout_ref.actor.policy_loss.loss_mode=alm.
#   * +distillation.distillation_loss.{alm_temperature,alm_f_divergence,
#                                     alm_debiasing} are passed through.
#   * `simple_loss_clamp` is RETAINED (the per-token rmpad clamp in
...
METHOD_DIR="${EXP_DIR}/methods/alm"
...
DISTILL_LOSS_MODE=alm
KL_DIRECTION=reverse
USE_POLICY_GRADIENT=False     # kd_ratio=1.0 -> pure distillation

# ALM-specific KD hyperparameters (KDFlow defaults).
# ALM_TEMPERATURE=100 is intentional: with binarised f-divergence,
# tau >= 50 selects the closed-form tau->infinity branch in
# easyopd.methods.alm.losses._binarised_f_divergence. Lowering this
# significantly (e.g. tau=1) would switch to the p^{1/tau} branch which
# may be numerically less stable.
ALM_TEMPERATURE=${ALM_TEMPERATURE:-100.0}
ALM_F_DIVERGENCE=${ALM_F_DIVERGENCE:-kl}      # kl | tvd
ALM_DEBIASING=${ALM_DEBIASING:-false}
...
echo "[$(date)] ===== Step 3: GRPO + ALM Cross-Tok KD Training ====="
...
${PYTHON} -m verl.trainer.main_ppo \
    ...
    +distillation.distillation_loss.alm_temperature=${ALM_TEMPERATURE} \
    +distillation.distillation_loss.alm_f_divergence=${ALM_F_DIVERGENCE} \
    +distillation.distillation_loss.alm_debiasing=${ALM_DEBIASING} \
    trainer.balance_batch=True \
    'trainer.logger=["console"]' \
    trainer.project_name=easyopd-alm \
