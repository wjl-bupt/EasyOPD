#!/usr/bin/env python3
"""
Code Evaluation Script for EasyOPD - powered by SGLang DP inference engine.
Supports: mbpp, live-code-bench-v6
...
Architecture:
    ...
    - Server must be started separately (see eval_code_sglang.sh for automation)
...
Usage:
    python evaluate_code_sglang.py --model_path /path/to/model --dataset mbpp
    python evaluate_code_sglang.py --model_path /path/to/model --dataset live-code-bench-v6
    python evaluate_code_sglang.py --model_path /path/to/model --dataset mbpp --output_dir /path/to/results
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="EasyOPD Code Evaluation (SGLang DP inference engine)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    # Start SGLang server first (in another terminal or via eval_code_sglang.sh)
...
def main():
    ...
    print("=" * 80)
    print("EasyOPD Code Evaluation (SGLang DP inference engine)")
    print("=" * 80)
    ...
    dataset_path = os.path.join(datasets_dir, args.dataset)
