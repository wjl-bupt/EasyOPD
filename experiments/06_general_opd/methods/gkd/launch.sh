...
#
# GKD combines on-policy sampling (student generates) with dense teacher
# feedback (per-token KL divergence), using a Generalized Jensen-Shannon
# Divergence (JSD) that interpolates between forward and reverse KL.
#
# Key features:
#   - On-policy: student generates sequences, avoiding distribution mismatch
#   - Dense teacher feedback: per-token KL divergence (not sparse reward)
#   - Generalized JSD: beta parameter interpolates forward/reverse KL
#   - Policy gradient integration: distillation loss as reward signal
#
# Pipeline:
#   1. Prepare RL prompt parquet (train.parquet / val.parquet)
#   2. (Re)start Ray
#   3. GRPO + GKD distillation via verl.trainer.main_ppo
#      with distillation.* enabled (teacher provides dense per-token feedback)
#   4. Merge each global_step_X/actor/ -> HF format
#   5. Evaluate every merged ckpt on math500 + gsm8k
#
# Teacher deployment: vLLM-based distillation resource pool (same-tokenizer).
...
METHOD_DIR="${EXP_DIR}/methods/gkd"
...
GKD_BETA=0.5                  # JSD interpolation: 0=forward KL, 0.5=symmetric, 1=reverse KL
GKD_TEMPERATURE=1.0           # Softmax temperature for KL computation
USE_POLICY_GRADIENT=True      # On-policy mode (GKD's key innovation)
USE_TASK_REWARDS=True         # Combine distillation with task rewards
DISTILLATION_LOSS_COEF=1.0    # Weight of distillation loss
LOSS_MODE="gkd"               # Generalized JSD loss

echo "[$(date)] GKD: beta=${GKD_BETA}, temperature=${GKD_TEMPERATURE}, policy_gradient=${USE_POLICY_GRADIENT}"
echo "[$(date)] GKD: grad_ckpt=${ENABLE_GRAD_CKPT}, rollout_mem=${ROLLOUT_GPU_MEM_UTIL}, teacher_mem=${TEACHER_GPU_MEM_UTIL}"
...
# ============================================================
# Step 3: GRPO + GKD Distillation Training
# ============================================================
...
echo "[$(date)] ===== Step 3: GRPO + GKD Distillation Training ====="
...
echo "GKD: beta=${GKD_BETA}, temperature=${GKD_TEMPERATURE}, loss_mode=${LOSS_MODE}"
...
${PYTHON} -m verl.trainer.main_ppo \
    ...
    +distillation.distillation_loss.use_cross_tokenizer=False \
    ...
    +distillation.distillation_loss.gkd_beta=${GKD_BETA} \
    +distillation.distillation_loss.gkd_temperature=${GKD_TEMPERATURE} \
    trainer.balance_batch=True \
    'trainer.logger=["console"]' \
    trainer.project_name=easyopd-gkd \
...
# ============================================================
echo "[$(date)] ===== Step 5: Evaluating GKD checkpoint(s) ====="
