...
# [EasyOPD] math_verify is an optional dependency (lasgroup/SDPO installs
# `math-verify[antlr4_9_3]==0.8.0`). When it is unavailable we fall back to the
# strict boxed-answer match only; install it for byte-faithful math rewards.
try:
...
def verify(
    ...
    # try Math-Verify equivalence check (skipped if math_verify is not installed)
    if not correct and pred != "" and _HAS_MATH_VERIFY:
        try:
